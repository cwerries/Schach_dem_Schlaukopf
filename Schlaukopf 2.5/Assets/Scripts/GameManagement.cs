...
public class GameManagement : MonoBehaviour
{
    ...
    // Speed of moving figures
    [SerializeField]
    public float figure_speed;

    public Vector3 destination;

    public Vector3 direction;

    public int moveslider;
    ...
    {
        RaycastHit hit;
        float raycastDistance = 100000.0f;
        ...
        else if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, raycastDistance, LayerMask.GetMask("QuitLayer")))
        {
            Debug.Log("On position Quit");
            // Quit from the game
            if (Input.GetMouseButtonDown(0))
            {
                Debug.Log ("QUIT!");
        ...
        else if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, raycastDistance, LayerMask.GetMask("MenuLayer")))
        {
            Debug.Log("On position Menu");
            if (Input.GetMouseButtonDown(0))
            {
    ...
    private void MoveFigure(int x, int y)
    {
        ...
        if (_allowedMoves[x, y])
        {
            ...
            StartCoroutine(Translate_Figure(_selectedFigure,x,y));
    ...
    private Vector3 TransitionFigure(int x, int y)
    {
        Vector3 origin = Vector3.zero;
        origin.x += (TILE_SIZE * x) + TILE_OFFSET;
        origin.z += (TILE_SIZE * y) + TILE_OFFSET;
    ...
    IEnumerator Translate_Figure(GameFigures _activeFigure, int x, int y)
    {
        Cursor.visible = false;

        float counter = 0;
        double calculated_speed = 0.0001 * ((figure_speed - 100) * (figure_speed - 100)) + 0.01;
        //Calculate the Vector3 destination on the grid
        destination = GetTileCenter(x, y);

        //Direction of movement equals destination minus the current position of a figure
        direction = destination - _activeFigure.transform.position;

        //Save the current orientation of a figure
        var original_rotation = _activeFigure.transform.rotation;

        //Rotate figure in the direction of the movement
        _activeFigure.transform.rotation = Quaternion.LookRotation(direction) * Quaternion.Euler(0, -90, 0);

        //Set moveslider to 0
        moveslider = 0;

        //This whileloop divides the euclidean distance between start position and destination of a figure in 100 steps
        //and iteratively move the figure to that direction

        

        while (moveslider != 20)
        {
            _activeFigure.transform.position = _activeFigure.transform.position + direction / 20;
            moveslider++;

            while (calculated_speed > counter)
            {
        ...
        //set back the original orientation of a figure
        _activeFigure.transform.rotation = original_rotation;
